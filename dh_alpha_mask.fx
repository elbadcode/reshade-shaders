...
// PS
    ...
    void PS_Apply(float4 vpos : SV_Position, float2 coords : TexCoord, out float4 outColor : SV_Target0) {
        ...
        float brightness = beforeColor.a;
		...
		mask = saturate(mask);
		if(iDebug==1) {
        	outColor = float4(mask,mask,mask, fAlphaScalar);
		} else if(iDebug==2) {
        	outColor = lerp(afterColor,float4(0,1,0,1),mask);
		} else {
        	outColor = lerp(beforeColor,afterColor,fAlphaScalar - mask); 
        }
    }
...
// TEHCNIQUES 
    ...
    technique DH_ALPHA_MASK_BEFORE<
        ui_label = "DH_ALPHA_MASK 0.3.1 BEFORE";
    > {
        pass {
    ...
    technique DH_ALPHA_MASK_AFTER<
        ui_label = "DH_ALPHA_MASK 0.3.1 AFTER";
    > {
        pass {
            VertexShader = PostProcessVS;
